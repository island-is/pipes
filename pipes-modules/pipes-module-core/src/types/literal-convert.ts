...
/**
 * hasManyItems
 * @desc Returns true if the type has more than one item.
 * @example
 * ```ts
 * const SingleItem: hasManyItems<string> = false;
 * const ManyItems: hasManyItems<string | number> = true;
 * ```
 *
 */
export type hasManyItems<
  InitalType,
  T = combineNullUndefined<InitalType>,
  ListArray extends List<any> = LiteralToArray<T>,
  ListLength = Length<ListArray>,
  IsModuleName extends boolean = InitalType extends string & { __type: "modulename" } ? true : false,
  Bool = isEmptyList<LiteralToArray<Exclude<T, boolean>>>,
